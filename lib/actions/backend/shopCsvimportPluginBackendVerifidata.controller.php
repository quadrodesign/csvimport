...
class shopCsvimportPluginBackendVerifidataController extends waLongActionController
{
  ...
  protected function init()
  {
    ...
    $path = shopCsvimportPlugin::path($this->data['info']['name_file']);
    $file = fopen($path, 'r');

    $this->data['header'] = $this->convert(fgetcsv($file, 0, ';'));
...
//      $pos = strpos((string)$header, 'features');
      if (is_string($header) && strpos((string)$header, 'features') !== false) {
        ...
        if ($ident[1] != '') {
          ...
          $feat = fopen(shopCsvimportPlugin::path('new' . ucfirst($ident[1]) . '.csv'), 'w');
    ...
    $upProd = fopen(shopCsvimportPlugin::path('updateProd.csv'), 'w');
    fputcsv($upProd, $this->reconvert($this->data['header']), ';');
    fclose($upProd);

    $ostat = fopen(shopCsvimportPlugin::path('ostat.csv'), 'w');
    fputcsv($ostat, $this->reconvert($this->data['header']), ';');
    fclose($ostat);

    $newProd = fopen(shopCsvimportPlugin::path('newProd.csv'), 'w');
    fputcsv($newProd, $this->reconvert($this->data['header']), ';');
    fclose($newProd);

    $upSkus = fopen(shopCsvimportPlugin::path('updateSkus.csv'), 'w');
    fputcsv($upSkus, $this->reconvert($this->data['header']), ';');
    fclose($upSkus);

    $newSkus = fopen(shopCsvimportPlugin::path('newSkus.csv'), 'w');
    fputcsv($newSkus, $this->reconvert($this->data['header']), ';');
    fclose($newSkus);

    $notFeat = fopen(shopCsvimportPlugin::path('notFeat.csv'), 'w');
    fputcsv($notFeat, $this->reconvert($this->data['header']), ';');
    fclose($notFeat);

    $notFoundFeat = fopen(shopCsvimportPlugin::path('notFoundFeat.csv'), 'w');
  ...
  protected function step()
  {
    ...
    $fp = fopen(shopCsvimportPlugin::path($this->data['info']['name_file']), 'r');
      ...
      if ($indicator >= $this->data['offset']) {
        if (($this->data['offset'] - $start) <= $limit) {
          ...
          if ($productId) {
            ...
            $upProd = fopen(shopCsvimportPlugin::path('updateProd.csv'), 'a');
            ...
            if ($skuId) {
              if ($this->data['info']['regim'] == 3) {
                if (is_array($this->data['skus']['stocks'])) {
                  ...
                  if ($i) {
                    $this->data['ostat']++;
                    $ostat = fopen(shopCsvimportPlugin::path('ostat.csv'), 'a');
                    if ($this->data['info']['charset'] == 'UTF-8') {
              ...
              $upSkus = fopen(shopCsvimportPlugin::path('updateSkus.csv'), 'a');
              if ($this->data['info']['charset'] == 'UTF-8') {
              ...
              $this->data['newSkus']++;
              $newSkus = fopen(shopCsvimportPlugin::path('newSkus.csv'), 'a');
              if ($this->data['info']['charset'] == 'UTF-8') {
            ...
            $newProd = fopen(shopCsvimportPlugin::path('newProd.csv'), 'a');
            if ($this->data['info']['charset'] == 'UTF-8') {
            ...
            foreach ($this->data['features'] as $key => $feature) {
              if (!empty($csvInfo[$feature['key']])) {
                if (is_numeric($this->data['info']['id_razmer'])) {
                  if ($feature['key'] == $this->data['info']['id_razmer'] && $productId) {
                    if ($size = shopTablesizePlugin::getSiteSize($productId, $csvInfo[$feature['key']])) {
                      ...
                      $this->data['notFeat']++;
                      $notFeat = fopen(shopCsvimportPlugin::path('notFeat.csv'), 'a');
                      if ($this->data['info']['charset'] == 'UTF-8') {
                    ...
                    $notFoundFeat = fopen(shopCsvimportPlugin::path('notFoundFeat.csv'), 'a');
                    if ($this->data['info']['charset'] == 'UTF-8') {
                ...
                if (!in_array($csvInfo[$feature['key']], $feature['values'])) {
                  ...
                  $ident = explode(':', $key);
                  $feat = fopen(shopCsvimportPlugin::path('new' . ucfirst($ident[1]) . '.csv'), 'a');
                  if ($this->data['info']['charset'] == 'UTF-8') {
  ...
  protected function report()
  {
    ...
    $report .= '<ul>';
    $report .= '<li><input type="checkbox" ' . $newProd . ' name="checkbox[newProd]"/><i class="icon16 yes"></i>' . count($this->data['newProduct']) . ' новых товара <a href="' . shopCsvimportPlugin::pathUrl('newProd.csv') . '" download>CSV</a><br></li>';
    $report .= '<li><input type="checkbox" ' . $updateProd . ' name="checkbox[updateProd]"/><i class="icon16 yes"></i>' . count($this->data['updateProduct']) . ' товара будут обновлены <a href="' . shopCsvimportPlugin::pathUrl('updateProd.csv') . '" download>CSV</a><br></li>';
    $report .= '<li><input type="checkbox" ' . $newSkus . ' name="checkbox[newSkus]"/><i class="icon16 yes"></i>' . $this->data['newSkus'] . ' новых артикула <a href="' . shopCsvimportPlugin::pathUrl('newSkus.csv') . '" download>CSV</a><br></li>';
    $report .= '<li><input type="checkbox" ' . $updateSkus . ' name="checkbox[updateSkus]"/><i class="icon16 yes"></i>' . $this->data['updateSkus'] . ' артикула будут обновлены <a href="' . shopCsvimportPlugin::pathUrl('updateSkus.csv') . '" download>CSV</a><br></li>';

    if ($this->data['ostat'] > 0) {
      $report .= '<li><i class="icon16 yes"></i>' . $this->data['ostat'] . ' артикула имеют негативный остаток <a href="' . shopCsvimportPlugin::pathUrl('ostat.csv') . '" download>CSV</a><br></li>';
    }
    ...
    if (isset($this->data['features'])) {
      foreach ($this->data['features'] as $f_id => $f) {
        if ($f['count'] > 0) {
          ...
          $report .= '<li><input type="checkbox" ' . $checked . ' name="checkbox[features][' . $f['code'] . ']"/><i class="icon16 yes"></i>' . $f['count'] . ' ' . $pole . ' с новой характеристикой типа "' . $f['name'] . '" <a href="' . shopCsvimportPlugin::pathUrl('new' . ucfirst($fid[1]) . '.csv') . '" download>CSV</a><br></li>';
        }
      }
    }

    if ($this->data['notFeat'] > 0) {
      $report .= '<li><i class="icon16 yes"></i>' . $this->data['notFeat'] . ' размеров не нашли соответствие в размерной тоблице <a href="' . shopCsvimportPlugin::pathUrl('notFeat.csv') . '" download>CSV</a>><br></li>';
    }

    if ($this->data['notFoundFeat'] > 0) {
      $report .= '<li><i class="icon16 yes"></i>' . $this->data['notFoundFeat'] . ' размеров не нашли соответствие в размерной тоблице(Новые товары)<a href="' . shopCsvimportPlugin::pathUrl('notFoundFeat.csv') . '" download>CSV</a><br></li>';
    }
