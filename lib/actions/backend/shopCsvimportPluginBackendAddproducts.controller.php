          ...
          if ($productId && $this->data['info']['checkbox']['updateProd']) {
            ...
            if (isset($this->data['features'])) {
              ...
              if ($not_update == false) {
                ...
                $feature_model->setData($p, $dataFeat);
                unset($dataFeat);
              }
            }
            ...
            if (is_array($this->data['skus']) && $not_update == false) {
              ...
              foreach ($this->data['skus'] as $key => $value) {
                if ($key == 'stocks') {
                  ...
                  } elseif ($this->data['info']['regim'] == 3 || $this->data['info']['regim'] == 5) {
                    $skuData = self::reduce_stock($csvInfo, $skuId);
                  }
                } else {
  ...
  protected function info()
  {
    ...
    if ($this->getRequest()->post('cleanup')) {
      if ($this->data['info']['name_file'] == 'addedSkus.csv') {
        waFiles::delete(shopCsvimportPlugin::path($this->data['info']['name_file']));
      }
